...
def run_high_dimensional_bb84_protocol_batched(key_length=256, dimension=4, backend=None):
    ...
    bob_binary = ''.join(f"{symbol:0{symbolBits}b}" for symbol in final_key_bob)
...
def create_transaction(
    ...
    For 4D, quantum key generation is done in a subprocess.
    """
    try:
    ...
    # Generate high-dimensional BB84 key using one-time circuit or subprocess
    quantum_dim = st.session_state.quantum_dimension
    if quantum_dim == 4:
        bb84_key = generate_4d_quantum_key_subprocess(256)
    else:
        bb84_key = one_time_circuit_high_dim_bb84(256, quantum_dim)
    if bb84_key is None:
    ...
    if ok:
        ...
        st.info(f" Used {quantum_dim}-dimensional quantum encoding")
...
# Helper: Run 4D quantum key generation in subprocess
def generate_4d_quantum_key_subprocess(key_length=256):
    """
    Run 4D quantum key generation in a subprocess to avoid blocking/crashing Streamlit.
    Returns the key as hex string, or None on failure.
    """
    import sys
    import subprocess
    import os
    ...
    script_path = os.path.join(os.path.dirname(__file__), "quantum_key_worker.py")
    try:
        result = subprocess.run(
            [sys.executable, script_path, str(key_length)],
        ...
        if result.returncode == 0:
            ...
            if len(key) >= 32:
                ...
                st.error("Quantum key subprocess returned invalid key.")
                return None
        else:
            st.error(f"Quantum key subprocess failed: {result.stderr.decode()}")
            return None
    except Exception as e:
        st.error(f"Quantum key subprocess error: {e}")
        return None
...
# ================== Part 4/5 ==================
    ...
    with st.sidebar:
        ...
        new_dimension = st.selectbox(
            "Quantum Dimension",
            [2, 4],  # Only 2D and 4D allowed
            index=[2, 4].index(st.session_state.quantum_dimension) if st.session_state.quantum_dimension in [2, 4] else 0,
